...
ImageRenderer::ImageRenderer() :
    ...
    transFlag(TRUE), turnXFlag(false), turnYFlag(false), drawNum(0u), image(this),
...
void ImageRenderer::Draw()
{
    ...
    DrawRotaGraph3(x, y, cx, cy, scale.x, scale.y, rotation, image.imageInfo->handles[drawNum],
        transFlag, (turnXFlag) ? TRUE : FALSE, (turnYFlag) ? TRUE : FALSE);

    SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);
}

bool ImageRenderer::SetImage(const std::string& _filePath)
{
...
bool ImageRenderer::SetImageDiv(const std::string& _filePath, unsigned _xdiv, unsigned _ydiv, unsigned _allnum)
{
    if (image.LoadDiv(_filePath, _xdiv, _ydiv, _allnum))
    {
        drawNum = 0u;
        return true;
    }
...
unsigned ImageRenderer::GetImageCount() const
{
